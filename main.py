...
def g_syn_hyper(g_syn_bar, a_syn, t):
    ...
    g_syn = Expression('g_syn_bar*exp(-fmod(t,0.02)/a_syn)*\
...
if __name__=='__main__':
    ...
    g_Na_leak = Constant(0.247/1.6)   # Na leak membrane conductivity (S/(m^2))
    g_K_leak = Constant(0.5/1.6)      # K leak membrane conductivity (S/(m^2))
    g_Cl_leak = Constant(0.5/2.5)     # Cl leak membrane conductivity (S/(m^2))

    I_max = 0.1804176                 # max pump strength (A/m^2)
    g_KCC2 = 0.068/20.0               # KCC2 cotransporter strength (A/m^2)
    ...
    params = {'dt':dt, 'Tstop':Tstop,
              ...
              'I_max':I_max, 'm_Na':m_Na, 'm_K':m_K,
              'sigma_i':sigma_i, 'sigma_e':sigma_e,
              'E_Na':E_Na, 'E_K':E_K, 'E_Cl':E_Cl,
              'g_KCC2':g_KCC2, 'g_NKCl':g_NKCl,
              'g_Na_leak':g_Na_leak, 'g_K_leak':g_K_leak, 'g_Cl_leak':g_Cl_leak,
              'D_A':D_A, 'A_i':A_i, 'A_e':A_e, 'z_A':z_A}
