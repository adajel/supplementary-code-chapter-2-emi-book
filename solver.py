...
class Solver:
    ...
    def create_variational_form(self, splitting_scheme=False, dirichlet_bcs=False):
        ...
        NKCl = 1./(1. + exp(16. - K_e)) * (ln((K_i*Cl_i)/(K_e*Cl_e)) + ln((Na_i*Cl_i)/(Na_e*Cl_e)))
    ...
    def solve_for_time_step(self):
        ...
        # NaKCl2 cotransporter
        u_NKCl = 1./(1. + exp(16. - K_e)) * (ln((K_i*Cl_i)/(K_e*Cl_e)) + ln((Na_i*Cl_i)/(Na_e*Cl_e)))
    ...
    def alpha_n(self, V_M):
        ...
        # rate coefficient
        return 0.01e3*(10.-V)/(exp((10.-V)/10.) - 1.)

    def beta_n(self, V_M):
        V_rest = self.params['V_rest']  # resting potential (V)
        V = 1000*(V_M - V_rest)         # convert from mV to V
        # rate coefficient
        return 0.125e3*exp(-V/80.)

    def alpha_m(self, V_M):
        V_rest = self.params['V_rest']  # resting potential (V)
        V = 1000*(V_M - V_rest)         # convert from mV to V
        # rate coefficient
        return 0.1e3*(25. - V)/(exp((25. - V)/10.) - 1)

    def beta_m(self, V_M):
        V_rest = self.params['V_rest']  # resting potential (V)
        V = 1000*(V_M - V_rest)         # convert from mV to V
        # rate coefficient
        return 4.e3*exp(-V/18.)

    def alpha_h(self, V_M):
        V_rest = self.params['V_rest']  # resting potential (V)
        V = 1000*(V_M - V_rest)         # convert from mV to V
        # rate coefficient
        return 0.07e3*exp(-V/20.)

    def beta_h(self, V_M):
        V_rest = self.params['V_rest']  # resting potential (V)
        V = 1000*(V_M - V_rest)         # convert from mV to V
        # rate coefficient
        return 1.e3/(exp((30.-V)/10.) + 1)
    ...
    def solve_system_HH(self, n_steps_ode, filename, dirichlet_bcs=False):
        ...
        V_rest = self.params['V_rest']          # resting potential (V)
        phi_M_init = self.params['phi_M_init']  # resting potential (V)
